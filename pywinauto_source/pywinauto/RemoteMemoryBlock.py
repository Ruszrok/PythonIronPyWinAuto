...
class RemoteMemoryBlock(object):
    ...
    def __init__(self, handle, size = 4096): #4096): #16384):
        ...
        self.memAddress = win32functions.VirtualAllocEx(
            ...
            win32structures.ULONG_PTR(size),# how much to allocate
    ...
    def CleanUp(self):
        ...
        if self.process != 0L and self.memAddress != 0L:
            # free up the memory we allocated
            #win32api.SetLastError(0)
            ret = win32functions.VirtualFreeEx(
                ctypes.c_void_p(self.process), ctypes.c_void_p(self.memAddress), win32structures.ULONG_PTR(0L), win32structures.DWORD(win32defines.MEM_RELEASE))
            if ret == 0:
                print('CleanUp: VirtualFreeEx() returned zero for address ', hex(self.memAddress))
            ...
            print('Cannot call VirtualFreeEx!!!')
    ...
    def Write(self, data, address = None, size = None):
        ...
        ret = win32functions.WriteProcessMemory(
            ...
            ctypes.pointer(data),
            nSize,
            win32structures.ULONG_PTR(0L));

        if ret == 0:
            print('Write failed: address = ', address)
            last_error = win32api.GetLastError()
            print('LastError = ', last_error, ': ', win32api.FormatMessage(last_error).rstrip())
            #raise ctypes.WinError()
    ...
    def Read(self, data, address = None, size = None):
        ...
        lpNumberOfBytesRead = ctypes.c_size_t(0L)
        ...
        ret = win32functions.ReadProcessMemory(
            ...
            ctypes.byref(data),
            nSize,
            ctypes.byref(lpNumberOfBytesRead)) #win32structures.ULONG_PTR(0L))
