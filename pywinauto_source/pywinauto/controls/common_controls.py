...
#        self.Check(check = False)
    ...
    def ClickInput(self):
        "Left click on the Toolbar button"
        self.toolbar_ctrl.ClickInput(button='left', coords = self.Rectangle().mid_point())
        time.sleep(Timings.after_toobarpressbutton_wait)
...
class ToolbarWrapper(HwndWrapper.HwndWrapper):
    ...
    #----------------------------------------------------------------
    def Button(self, button_identifier):
...
#
    ...
    def PressButton(self, button_identifier):
        ...
        button = self.Button(button_identifier) #button_info.idCommand) #button_index)
    ...
    #----------------------------------------------------------------
    def CheckButton(self, button_identifier, make_checked):
        ...
        if isinstance(button_identifier, basestring):
            ...
            button_index = button_identifier
            if make_checked:
                self.actions.log('Pressing down toolbar button number ' + str(button_index) + ' ("' + str(texts[1:][button_index]) + '")')
            else:
                self.actions.log('Pressing up toolbar button number ' + str(button_index) + ' ("' + str(texts[1:][button_index]) + '")')

        #button_info = self.GetButton(button_index)
        button = self.Button(button_index) #button_info.idCommand)
